...
- Phân quyền truy cập dataset.
- Phân công dataset môn học cho giảng viên.
...
| `DatasetPermissions` | Lưu quyền truy cập Dataset (riêng tư) cho người dùng |
| `TeacherSubjectAssignments` | Phân công Dataset (môn học) quản lý cho Giảng viên |
...
| `password_hash` | VARCHAR | Mật khẩu băm bảo mật |
| `role` | VARCHAR | Vai trò người dùng (Admin, Teacher, Student) |
| `is_approved` | BOOLEAN | Trạng thái phê duyệt tài khoản bởi Admin |
| `must_change_password` | BOOLEAN | Yêu cầu đổi mật khẩu ở lần đăng nhập đầu tiên |
| `temporary_password_expires_at` | TIMESTAMP | Thời hạn hết hạn mật khẩu tạm thời |
| `last_password_changed_at` | TIMESTAMP | Thời điểm đổi mật khẩu lần cuối |
...
| `is_public` | BOOLEAN | Chế độ công khai (True) hoặc riêng tư (False) |
| `is_approved` | BOOLEAN | Trạng thái phê duyệt dataset bởi Admin |
...
## 3.9. Bảng DatasetPermissions

Bảng `DatasetPermissions` lưu thông tin phân quyền truy cập các Dataset (riêng tư) cho từng người dùng cụ thể.
...
| Trường | Kiểu dữ liệu | Ý nghĩa |
|---|---|---|
| `permission_id` | UUID | Khóa chính của bản ghi quyền |
| `dataset_id` | UUID | Dataset được phân quyền, liên kết với bảng `Datasets` |
| `user_id` | UUID | Người dùng được cấp quyền, liên kết với bảng `Users` |
| `granted_at` | TIMESTAMP | Thời điểm cấp quyền |

### Vai trò trong hệ thống

Bảng này hỗ trợ cơ chế bảo mật và phân quyền chi tiết. Khi một dataset không được đánh dấu là công khai (`is_public = false`), chỉ những người dùng có bản ghi tương ứng trong bảng `DatasetPermissions` mới có quyền xem tài liệu và hỏi đáp chatbot dựa trên dataset đó.

---

## 3.10. Bảng TeacherSubjectAssignments

Bảng `TeacherSubjectAssignments` lưu thông tin phân công Dataset (môn học/chủ đề) cho Giảng viên (Teacher) để họ quản lý.
...
| Trường | Kiểu dữ liệu | Ý nghĩa |
|---|---|---|
| `assignment_id` | UUID | Khóa chính của bản ghi phân công |
| `teacher_id` | UUID | Giảng viên được phân công quản lý dataset, liên kết với bảng `Users` |
| `dataset_id` | UUID | Dataset được phân công, liên kết với bảng `Datasets` |
| `assigned_by` | UUID | Admin thực hiện phân công, liên kết với bảng `Users` |
| `assigned_at` | TIMESTAMP | Thời điểm phân công |

### Vai trò trong hệ thống

Bảng này dùng để quản lý quyền sở hữu/quản trị của giảng viên đối với các dataset. Một dataset chỉ có thể được phân công cho một giảng viên duy nhất quản lý (quan hệ 1-1 giữa Dataset và TeacherSubjectAssignment). Giảng viên được phân công sẽ có toàn quyền thêm, sửa, xóa tài liệu trong dataset đó, trong khi admin có thể giám sát tất cả.
...
```mermaid
erDiagram
    Chunks ||--|| VectorRecords : embedded_as
```
...
```mermaid
erDiagram
    Datasets ||--o{ ChatSessions : used_in
```
...
## 4.12. Datasets - DatasetPermissions

```mermaid
erDiagram
    Datasets ||--o{ DatasetPermissions : has
```

Một dataset có thể có nhiều người dùng được cấp quyền truy cập riêng tư.  
Mỗi bản ghi quyền tương ứng với duy nhất một dataset.

Quan hệ:

```text
Datasets 1 - N DatasetPermissions
```

---

## 4.13. Users - DatasetPermissions

```mermaid
erDiagram
    Users ||--o{ DatasetPermissions : receives
```

Một người dùng có thể được cấp quyền trên nhiều dataset riêng tư khác nhau.  
Mỗi bản ghi quyền thuộc về một người dùng duy nhất.

Quan hệ:

```text
Users 1 - N DatasetPermissions
```

---

## 4.14. Datasets - TeacherSubjectAssignments

```mermaid
erDiagram
    Datasets ||--|| TeacherSubjectAssignments : managed_by
```

Mỗi dataset (môn học/chủ đề) chỉ có tối đa một giảng viên được phân công làm người quản lý (quan hệ 1-1).  
Mỗi bản ghi phân công tương ứng với một dataset duy nhất.

Quan hệ:

```text
Datasets 1 - 1 TeacherSubjectAssignments
```

---

## 4.15. Users - TeacherSubjectAssignments

```mermaid
erDiagram
    Users ||--o{ TeacherSubjectAssignments : assigned_to
```

Một giảng viên (User có role Teacher) có thể được phân công quản lý nhiều dataset (chủ đề/môn học) khác nhau.  
Mỗi bản ghi phân công chỉ định duy nhất một giảng viên.

Quan hệ:

```text
Users 1 - N TeacherSubjectAssignments
```
